    ...
    public class Oracle_v1Controller : ApiController
    {
        ...
        public EQResultTable_v1 v1_Execute(UWA_BODY uwa_body)
        {
            ...
            if (ModelState.IsValid && uwa_body != null)
            {
                ...
                var inParams = new List<OracleParameter>();
                foreach (PARAM param in uwa_body.PARAM_LIST)
                {
                    inParams.Add(new OracleParameter(param.TEXT, param.VALUE));
                }
                //GET
                if (uwa_body.METHOD == AppKeys_v1.RETURN_TYPE_GET)
                {
                    return Services.Oracle.Execute_v1.ExecuteQuery(payload.DB_CONNECTION, sql, inParams.ToArray());
                }
                //POST
                else if (uwa_body.METHOD == AppKeys_v1.RETURN_TYPE_POST)
                {
                    ...
                    EQResult_v1 dbObj = Services.Oracle.Execute_v1.ExecuteSF(payload.DB_CONNECTION, new List<SQL_PLIST_v1> { obj });
