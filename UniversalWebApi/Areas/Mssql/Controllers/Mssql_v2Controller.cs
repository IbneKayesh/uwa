...
{
    public class Mssql_v2Controller : ApiController
    {
        [HttpPost]
        [Route("api/Mssql/v2/Execute")]
        ...
        public EQResultSet_v2 v2_Execute(UWA_BODY_M uwa_body)
        {
            if (ModelState.IsValid && uwa_body != null)
            {
                ...
                List<UWA_CONNECTION> ConnectList = Services.Db.ApiStore.getActiveConnection(header.BRANCH_TOKEN, header.PAYLOAD_TOKEN);
                ...
                foreach (UWA_BODY_D item in uwa_body.DATA_BODY)
                {
                    ...
                    obj.SQL = Services.Db.ApiStore.GetSqlFile(item.RESOURCE, payload.PAYLOAD_TABLE);
                ...
                {
                    return Services.Mssql.Execute_v2.ExecuteQuery(payload.DB_CONNECTION, _sqlParam);
                }
